...
module app_appsettings './site/config--appsettings/main.bicep' = if (!empty(appSettingsKeyValuePairs)) {
...
module app_authsettingsv2 './site/config--authsettingsv2/main.bicep' = if (!empty(authSettingV2Configuration)) {
...
@batchSize(1)
module app_slots './site/slot/main.bicep' = [for (slot, index) in slots: {
...
module app_hybridConnectionRelays './site/hybrid-connection-namespace/relay/main.bicep' = [for (hybridConnectionRelay, index) in hybridConnectionRelays: {
